...
The two-path structure exists because Claude cannot generate images and a skill
that pretends otherwise produces confident nonsense. The most common failure
mode is an agent describing an image it never made. Naming the path in the first
line of the response is the cheapest fix for that.

The read-it-back loop is the part that earns this variant its existence. Claude
can view an image file it just wrote, which means it can catch its own broken
hierarchy, clipped text, and unreadable-at-feed-size type before a human ever
opens the file. Skipping that step reduces the skill to a description of a
thumbnail. Downscaling a copy to roughly 320px before the second look matters
too, because a defect that is obvious at 1280px wide is usually not the one that
kills the click.
